	...
	*/
	...
	loadNode(id, nodeElement) {
		...
		if (obj == null) {
			...
			// check the type is the same as the one previously loaded
			if (obj.type != nodeElement.type && obj.type != "lod") {
				obj.type = nodeElement.type;
			}
		}
		...
		// load position if node type is powerup
		if (nodeElement.type === "powerup") {
			obj['position'] = this.getVector3(nodeElement, "position");
		}

		// load obstacle positions if node type is obstacle
		if (nodeElement.type === "obstacle") {
			obj['position'] = [];
			for (let i in nodeElement["obstacle_positions"]) {
				obj['position'].push(this.getVectorN(nodeElement["obstacle_positions"][i], ["x", "y", "z"]));
			}
		}

		// load cloud positions if node type is cloud
		if (nodeElement.type === "cloud") {
			obj['position'] = [];
			for (let i in nodeElement["cloud_positions"]) {
				obj['position'].push(this.getVectorN(nodeElement["cloud_positions"][i], ["x", "y", "z"]));
			}
		}
